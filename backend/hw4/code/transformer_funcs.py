...
class Multi_Headed(tf.keras.layers.Layer):
	def __init__(self, emb_sz, use_mask):
		...
		self.key_weight1 = self.add_weight("w_K-1-" + id_generator(), shape=(emb_sz, sizes[0]))
		self.value_weight1 = self.add_weight("w_V-1-" + id_generator(), shape=(emb_sz, sizes[0]))
		self.query_weight1 = self.add_weight("w_Q-1-" + id_generator(), shape=(emb_sz, sizes[0]))

		self.key_weight2 = self.add_weight("w_K-2-" + id_generator(), shape=(emb_sz, sizes[1]))
		self.value_weight2 = self.add_weight("w_V-2-" + id_generator(), shape=(emb_sz, sizes[1]))
		self.query_weight2 = self.add_weight("w_Q-2-" + id_generator(), shape=(emb_sz, sizes[1]))

		self.key_weight3 = self.add_weight("w_K-3-" + id_generator(), shape=(emb_sz, sizes[2]))
		self.value_weight3 = self.add_weight("w_V-3-" + id_generator(), shape=(emb_sz, sizes[2]))
		self.query_weight3 = self.add_weight("w_Q-3-" + id_generator(), shape=(emb_sz, sizes[2]))
...
class Position_Encoding_Layer(tf.keras.layers.Layer):
	def __init__(self, window_sz, emb_sz):
		...
		self.positional_embeddings = self.add_weight("pos_embed-" + id_generator(),shape=[window_sz, emb_sz])
